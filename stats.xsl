...
Strong attackers: <xsl:value-of select="count(pokemon[attack/text() > 150])" /> <xsl:text>
</xsl:text>
<xsl:apply-templates select="pokemon[attack/text() > 150]" />
    
Strong defenders: <xsl:value-of select="count(pokemon[defense/text() > 150])" /> <xsl:text>
</xsl:text>
<xsl:apply-templates select="pokemon[defense/text() > 150]" />
    
Fast: <xsl:value-of select="count(pokemon[speed/text() > 150])" /> <xsl:text>
</xsl:text>
